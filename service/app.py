...
def endresult(data):
    ...
    # for i in data:
    #     print(type(i))
    k[0]=float(data['loan_amnt'])
    k[1]=data['term']              #in months
    k[2]=float(data['int_rate'])
    k[3]=float(data['installment'])
    k[4]=float(data['emp_length'])
    k[5]=data['dti']                #debt to income ratio
    k[6]=float(data['earliest_cr_line'])   #date (YYYY)
    k[7]=float(data['open_acc'])
    k[8]=float(data['pub_rec'])
    k[9]=float(data['revol_util'])
    k[10]=float(data['total_acc'])
    k[11]=float(data['mort_acc'])
    k[12]=float(data['pub_rec_bankruptcies'])
    k[13]=np.log10(float(data['annual_inc'])+1)   #income
    k[14]=float(data['fico_score'])
    k[15]=np.log10(float(data['revol_bal'])+1)
    ...
    k[grades[data['grade']]]=1
    
    home={"OTHER":50,"OWN":51,"RENT":52}
    
    k[home[data['home_ownership']]]=1
    
    verification={"Source Verified":53,"Verified":54}
    
    k[verification[data['verification_status']]]=1
    
    purposes={"credit card":55,"debt consolation":56,"educational":57,"home improvement":58,"house":59,"major purchase":60
            ,"medical":61,"moving":62,"other":63,"renewable energy":64,"small business":65,"vacation":66,
            "wedding":67}
    
    k[purposes[data['purpose']]]=1
    ...
    k[states[data['state']]]=1
    
    #initial listing status
    k[118]=1
    
    if data['application_type']=="joint":
        k[119]=1
    else:
        k[119]=0
    ...
    if result[0]==0: 
        answer = "Fully Paid :: High chances of loan repayment with probability of "+str(result_prob[0])
    else:
        answer = "Charged Off:: The Loan is unlikely to be repaid with probability of "+str(result_prob[0])
...
class MainClass(Resource):
    ...
    def post(self):
        try: 
            ...
            print("Hii RA",formData['int_rate'])
